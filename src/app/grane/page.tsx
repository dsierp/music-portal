...
export default async function GranePage() {
  ...
  const [odsluchy, plyty, topki, lubiane] = await Promise.all([
    ostatnieOdsluchy(user.id, 60).catch(() => []),
    ...
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      {!odsluchy.length ? (
        ...
        <>
          ...
          <section>
            <h2 className="label mb-2">{t.lists.playedTitle}</h2>
              ...
              {odsluchy.map((o) => (
                <li key={o.id} className="flex items-baseline justify-between gap-3 py-2">
                  ...
                  <span className="shrink-0 font-mono text-[10px] text-faint">
                    ...
                    {o.source === "klik" ? ` · ${t.lists.playedFrom}` : ""}
                  </span>
                </li>
              ))}
            </ul>
